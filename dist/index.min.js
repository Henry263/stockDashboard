...
//$(document).ready(function() {
    ...
    function check_valid_label(label_to_check, _newstockData){

        var validate_labl = 'https://query.yahooapis.com/v1/public/yql?q=select * from yahoo.finance.quotes where symbol in ("' + label_to_check + '")&format=json&diagnostics=true&env=store://datatables.org/alltableswithkeys';
    ...
    function callAPI(apiStringVal, stocksObjectVal) {
        ...
        if (apiStringVal) {
            ...
            var testurl = 'https://finance.google.com/finance/info?client=ig&q=nasdaq:' + apiStringVal + '&callback=?';
            console.log(testurl);
            $.ajax({
            ...
            if (stockAryFlag) {
                ...
                if(getstcFlag)
                {
                    ...
                    if(keyFlag)
                    {
                        ...
                        check_valid_label(sName, stocktable);
    ...
    function deletestock(_stocname){
        ...
        if(validkeyFlag)
        {
            ...
            $.ajax({
                ...
                url: 'https://stok-dashboard.herokuapp.com/deleteStock',
                data: _deletedata,
                dataType: "json",
                crossDomain: true,
                complete: function() {
    ...
    function appendstockData(_appndStcData){
        ...
        else
        {
            ...
            console.log("Key is already present in JSON file.");
        }
    }
    ...
    function getnewsfeed(lblArray)
    {
        ...
        var each_item = '<div class="each_news_holder">' +
            ...
            '</div>' +
            '</div>';
        $.each(lblArray, function (key, val) {
            $.get('https://www.google.com/finance/company_news?q='+val+'&output=rss', function (data) {
                news_obj[val] = {};
                var itemcount = 0;
                ...
                $('.nw_container').find('.each_news_holder').each(function(){
                    ...
                    /*------------------
                    var linkTxt = desc.find('a').text();
                    linkTxt = linkTxt.substring(0,60) + "...";
                    desc.find('a').text(linkTxt);
                    -----------------------*/
    ...
    function getlabelsfornews(_usrkey_labels){
        var isKeyNull = checkUnqKey(_usrkey_labels);
        if(isKeyNull)
        {
            console.log(_usrkey_labels);
            $.ajax({
                type: "GET",
                dataType: "json",
                data: '',
                contentType: "application/json; charset=utf-8",
                url: 'https://stok-dashboard.herokuapp.com/getUsrKey/'+_usrkey_labels,
                complete: function() {
                    //called when complete
                ...
                success: function(res) {
                    ...
                    if (isEmpty(res)) {
                        ...
                        if (isEmpty(res)) {
                            ...
                            // Data found and need to manulapte.
                            console.log("Call from the labels and news response: "+res);
                            $.each(res, function (index, value) {
                                getLblsArry.push(index);
                            });
                            getnewsfeed(getLblsArry);
                        }
                    }
                },
                error: function() {
            ...
            console.log(getLblsArry);
        }
        else
        {
            // User key is not matching or not present in local storage so need to check on json file.;
            console.log("User key is not found.");
        }
    }
    validate_userkey();
    function validate_userkey() {
        var get_local_storage = localStorage.getObject("userUniqKey123");
        getlabelsfornews(get_local_storage);
